...
public class TripleTriadGUI extends JFrame {
    ...
    public TripleTriadGUI(Jogador jogador, Jogo jogo) {
        ...
        if (this.jogo != null) {
            ...
            // Considerar lançar uma exceção ou fechar a aplicação educadamente
        }
    ...
    private void initializePanels() {
        if (jogo == null) {
            ...
            // Adicionar JLabels vazios para evitar NullPointerException se a GUI tentar renderizar
            lblPlayer1Score = new JLabel("P1: -");
            lblPlayer2Score = new JLabel("P2: -");
            lblTurnIndicator = new JLabel("Erro no jogo");
            // Configurar outros painéis como vazios ou com mensagens de erro
            return;
        }
        ...
        int opponentHandSize = (oponente != null && oponente.getCartasNaMao() != null) ? 5 : 0; // Ou o real tamanho se variar
        ...
        // Painel Esquerdo (Cartas visíveis do oponente)
        List<Carta> opponentVisibleCardsList = new ArrayList<>();
        if (oponente != null && this.jogador != null) { // this.jogador é o viewer
    ...
    public void boardSlotClicked(int row, int col) {
        ...
        if (jogo.getJogadorAtual() == this.jogador && this.selectedCardFromHand != null && !jogo.jogoFinalizado()) {
            ...
            // Se sucesso=true, Jogo.notifyObservers() que está em tentarJogarCarta chamará updateGUI.
            ...
            if(jogo.getTabuleiro() != null && tabuleiroVazio(jogo.getTabuleiro())){
                // Não faz nada ou mostra info do slot se clicar em slot vazio com tabuleiro vazio
            } else {
    ...
    public void updateGUI() {
        SwingUtilities.invokeLater(() -> {
            ...
            // Os outros painéis (northPanelContainer, rightOpponentReservePanel) são mais estáticos
            // e são configurados em initializePanels. Seus conteúdos (JLabels de status) são
            // atualizados por refreshStatusDisplay. A mão oculta do oponente também é estática (5 cartas).
            ...
            if (oponente != null && this.jogador != null) {
                List<Carta> initiallyRevealed = jogo.getReveladasDoOponenteParaJogador(this.jogador);
                List<Carta> opponentCurrentHand = oponente.getCartasNaMao(); // Pode ser null se oponente não inicializado
                if (initiallyRevealed != null && opponentCurrentHand != null) {
                    for (Carta revealedCard : initiallyRevealed) {
                        if (opponentCurrentHand.contains(revealedCard)) {
                            opponentVisibleCardsList.add(revealedCard);
    ...
    private void refreshStatusDisplay() {
        ...
        // Checagens de nulidade para j1 e j2 antes de acessar seus métodos
